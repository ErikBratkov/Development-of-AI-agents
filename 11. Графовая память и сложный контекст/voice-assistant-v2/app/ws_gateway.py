...
from app.knowledge import wait_warmup, warmup_pending
...
async def notify_when_kb_ready(session: WsSession) -> None:
    """Сообщает клиенту о готовности модели поиска после прогрева"""
    await wait_warmup()
    await session.send_json({"type": "kb_ready"})
...
async def handle_websocket(
    ...
    # пока грузится модель поиска, клиент блокирует ввод, о конце
    # загрузки ему сообщит отдельная фоновая задача
    notify_task: asyncio.Task | None = None
    if warmup_pending():
        await session.send_json({"type": "kb_loading"})
        notify_task = asyncio.create_task(notify_when_kb_ready(session))
    try:
        while True:
