...
const sendBtn = textForm.querySelector('button')
const hintEl = document.getElementById('hint')

let ws = null
let wsReady = false
// сервер грузит модель поиска, ввод на это время блокируется
let kbLoading = false
...
function updateControls () {
  recBtn.disabled = !(wsReady && micReady) || kbLoading
  textInput.disabled = !wsReady || kbLoading
  sendBtn.disabled = !wsReady || kbLoading
}
...
function handleEvent (msg) {
  switch (msg.type) {
    ...
    case 'kb_loading':
      kbLoading = true
      updateControls()
      setStatus('Загружается модель поиска, подождите ...')
      break
    case 'kb_ready':
      kbLoading = false
      updateControls()
...
async function initMic () {
  ...
  // сборка аудио-графа тоже может упасть (worklet не загрузился,
  // AudioContext запрещен) - тогда честно остаемся в текстовом режиме
  try {
    ...
    hintEl.textContent =
      'не удалось инициализировать захват звука, работает только текстовый ввод'
    return
  }
  micReady = true
  updateControls()
...
textForm.addEventListener('submit', (event) => {
  ...
  if (!text || !wsReady || kbLoading) return
  if (playing) {
    sendJson({ type: 'barge_in' })
    stopPlayback()
  }
